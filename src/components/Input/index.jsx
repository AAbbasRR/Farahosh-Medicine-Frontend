...
export const InputCustom = forwardRef(function (props, ref) {
	...
	return (
		<div className={`${style.input} ${sizeClass} ${typeClass} ${className} ${leftIcon ? 'hasLeftIcon' : ''} ${rightIcon ? 'hasRightIcon' : ''} ${error ? 'error' : ''}`}>
			{label && (
			...
			{(error || helperText || counter) && (
				<div className={style.input__help}>
					{(helperText || error) && <span className={style.input__helpText}>{error || helperText}</span>}
